...
const AdvancedSignupDialog = ({ open, onOpenChange, onSuccess }: AdvancedSignupDialogProps) => {
  ...
  const handleAdvancedSignup = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-crypto-dark border-crypto-gray">
        <DialogHeader>
          <DialogTitle className="text-2xl text-crypto-blue">Sign Up for Advanced Plan</DialogTitle>
          <DialogDescription className="text-gray-300">
            Create your account to access advanced features
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleAdvancedSignup} className="space-y-4">
          <div>
